...
const onRegionChange = async () => {
  ...
  try {
    ...
    // Combine and sort
    const combined = [...provData, ...cityData].sort((a: any, b: any) =>
...
const renderMarkers = () => {
  ...
  const filtered =
    selectedAgency.value === "All Agencies"
...
const searchArea = async (force = false) => {
  ...
  const query = `
    [out:json][timeout:15];
    (
      node["name"~"DSWD|PhilHealth|Pag-IBIG|Pag IBIG|SSS|Social Security System|GSIS|Government Service Insurance|DOLE|Department of Labor|DOH|Department of Health|OWWA|Overseas Workers",i](${bbox});
      way["name"~"DSWD|PhilHealth|Pag-IBIG|Pag IBIG|SSS|Social Security System|GSIS|Government Service Insurance|DOLE|Department of Labor|DOH|Department of Health|OWWA|Overseas Workers",i](${bbox});
    );
    out center;
  `;
  ...
  try {
    ...
    const newBranches: Branch[] = data.elements.map((el: any) => {
      ...
      else if (nameLower.includes("pag-ibig") || nameLower.includes("pag ibig"))
        agency = "Pag-IBIG";
      else if (nameLower.includes("sss") || nameLower.includes("social security system"))
        agency = "SSS";
      else if (nameLower.includes("gsis") || nameLower.includes("government service insurance"))
        agency = "GSIS";
      else if (nameLower.includes("dole") || nameLower.includes("department of labor"))
        agency = "DOLE";
      else if (nameLower.includes("doh") || nameLower.includes("department of health"))
        agency = "DOH";
      else if (nameLower.includes("owwa") || nameLower.includes("overseas workers"))
        agency = "OWWA";
  ...
  <main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mb-20 md:mb-8">
    ...
    >
      <div class="flex items-center gap-3 mb-2">
        <div
          class="p-2.5 bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 rounded-xl"
        >
    ...
    >
      ...
      <div class="flex justify-between items-center mb-4 flex-wrap gap-3">
        ...
        >
          <span class="text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500 mr-1"
            >Legend</span
          >
          ...
          <div class="flex items-center gap-1.5">
            <span class="w-2.5 h-2.5 rounded-full bg-purple-600 shadow-sm"></span>
            <span>GSIS</span>
          </div>
          <div class="flex items-center gap-1.5">
      ...
      >
        ...
        <p
          v-if="fetchedBranches.length > 0"
          class="px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded-md whitespace-nowrap"
        >
          Found {{ fetchedBranches.length }} branches cached.
        </p>
      </div>
    </motion.div>
  </main>
