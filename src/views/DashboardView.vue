...
// Expanded cards for accordion (still works if they don't want to navigate, but we'll adapt it)
...
// Expansion logic removed in favor of routing
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        <section class="lg:col-span-3 space-y-6">
          ...
          <ProgramCard v-for="program in filteredPrograms" :key="program.id" :program="program"
            :is-saved="savedPrograms.includes(program.id)" :is-expanded="expandedProgramId === program.id"
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <motion.div v-for="program in shortlistedPrograms" :key="program.id"
          ...
          <div class="mt-4">
            ...
            <div
              class="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden border border-slate-200/50 dark:border-slate-800/50">
              <div class="bg-blue-900 dark:bg-blue-800 h-full transition-all duration-300"
                :style="{ width: `${getPreparedPercent(program)}%` }"></div>
            </div>
          </div>

          <!-- View details button -->
          <motion.button @click="goToProgram(program.id)"
