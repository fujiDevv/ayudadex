...
<template>
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 animate-fade-in">
    <div class="space-y-6">
      ...
      <div v-if="store.shortlistedPrograms.length === 0" class="text-center py-20 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm mt-8">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <ProgramCard v-for="program in store.shortlistedPrograms" :key="program.id" :program="program"
          :is-saved="true" :is-expanded="false" :checked-requirements="store.checkedRequirements[program.id] || []"
