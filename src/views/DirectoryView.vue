...
const categoriesList = computed(() =>
  Array.from(new Set(programsData.map((p) => p.category.split(" / ")).flat())).sort(),
);
const audienceTags = [
  "Indigent",
  "Senior Citizen",
...
const filteredPrograms = computed<Program[]>(() => {
  return (programsData as Program[]).filter((program) => {
    ...
    const matchesAgency =
      selectedAgencies.value.length === 0 || selectedAgencies.value.includes(program.agency);
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 animate-fade-in space-y-10">
    ...
    >
      ...
      <div class="relative z-10 max-w-3xl space-y-6">
        ...
        <div class="pt-4 max-w-xl relative z-35">
          ...
          >
            ...
            <div class="overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800">
              <div
                v-for="prog in filteredPrograms.slice(0, 5)"
                :key="prog.id"
                @mousedown="goToProgram(prog.id)"
              ...
              >
                <div class="flex items-center justify-between gap-2">
                  <span
                    class="text-xs font-bold text-slate-900 dark:text-slate-100 line-clamp-1 flex-grow"
                  >
                    {{ $t(`programs.${prog.id}.name`) }}
                  </span>
                  <span
                    :class="getAgencyStyles(prog.agency)"
            ...
            <div
              v-if="filteredPrograms.length > 5"
              @mousedown="scrollToResults"
        ...
        <div
          class="grid grid-cols-3 gap-4 pt-6 max-w-lg border-t border-slate-200 dark:border-slate-800/60 text-left"
        >
          <div>
            <span
              class="block text-2xl sm:text-3xl font-extrabold text-blue-600 dark:text-blue-400"
              >{{ programsData.length }}</span
            >
            <span
              class="text-[10px] sm:text-xs font-bold text-slate-500 dark:text-slate-500 uppercase tracking-wide"
              >Assistance Programs</span
            >
          </div>
          <div>
            <span
              class="block text-2xl sm:text-3xl font-extrabold text-teal-600 dark:text-teal-400"
              >{{ agenciesList.length }}</span
            >
            <span
              class="text-[10px] sm:text-xs font-bold text-slate-500 dark:text-slate-500 uppercase tracking-wide"
              >Agencies Covered</span
            >
          </div>
          <div>
            <span
              class="block text-2xl sm:text-3xl font-extrabold text-amber-600 dark:text-amber-400"
    ...
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <motion.button
          v-for="vc in visualCategories"
          :key="vc.id"
          @click="toggleVisualCategory(vc.tags)"
    ...
    >
      ...
      <!-- Audience Tags -->
      <span
        v-for="aud in selectedAudiences"
        :key="aud"
        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-100 dark:bg-slate-850 text-slate-700 dark:text-slate-300 text-xs font-bold border border-slate-200/40 dark:border-slate-800"
      >
        {{ aud }}
    ...
    <div
      ...
      class="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start scroll-mt-24"
    >
      <!-- Refinement Sidebar -->
      <motion.aside
      ...
      >
        ...
        <div class="space-y-2.5">
          <label
            class="text-[10px] font-bold tracking-wider uppercase text-slate-400 dark:text-slate-500 block mb-1"
          >
            {{ $t("filters.agency") }}
          </label>
              ...
              <span
                ...
                :class="
                  selectedAgencies.includes(agency)
                    ...
                    : 'bg-slate-100 dark:bg-slate-900 text-slate-500 dark:text-slate-400 border border-slate-200/10 dark:border-slate-800/10'
                "
              >
      ...
      <section class="lg:col-span-3 space-y-6">
        ...
        >
          ...
          >
            <button
              ...
              :class="
                viewMode === 'grid'
                  ? 'bg-white dark:bg-slate-900 text-blue-600 dark:text-blue-400 shadow-sm border border-slate-200/20 dark:border-slate-800/20'
        ...
        >
          <ProgramCard
            ...
            :key="program.id"
            :program="program"
            :is-saved="store.savedPrograms.includes(program.id)"
            :is-expanded="false"
