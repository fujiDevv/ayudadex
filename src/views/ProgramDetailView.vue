...
import {
  ...
  Share2, Check, ExternalLink, Printer, Lightbulb
...
<template>
  <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 mb-16 animate-fade-in">
    ...
    <div v-if="!program"
      ...
      <button @click="goBack"
        class="mt-6 px-6 py-2.5 bg-blue-900 text-white font-bold rounded-xl hover:bg-blue-800 transition-colors cursor-pointer">{{
          $t('detail.returnHome') }}</button>
    </div>
    ...
    <div v-else
      ...
      <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
        <div class="space-y-5">
          <div class="flex items-center gap-2 flex-wrap">
            <span
              class="px-2.5 py-0.5 bg-blue-100 dark:bg-blue-950/40 text-blue-900 dark:text-blue-300 text-[10px] font-bold uppercase rounded-md tracking-wider border border-blue-200/50 dark:border-blue-900/30">
              {{ program.agency }}
            </span>
            <span
              class="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-[10px] font-bold uppercase rounded-md tracking-wider">
              {{ program.category }}
            </span>
          </div>
          <h1
            class="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight leading-none text-slate-900 dark:text-white sm:from-slate-900 sm:via-slate-800">
        ...
        <div class="flex items-center gap-2 sm:gap-3 shrink-0">
          <motion.button @click="printChecklist"
            ...
            :title="$t('detail.print') || 'Print'" :aria-label="$t('detail.print') || 'Print'"
            :whileHover="{ scale: 1.05 }" :whileTap="{ scale: 0.95 }">
            <Printer class="w-5 h-5" />
            <span class="hidden sm:inline">{{ $t('detail.print') || 'Print' }}</span>
          </motion.button>

          <motion.button @click="shareProgram"
            ...
            :title="$t('detail.share')" :aria-label="$t('detail.share')" :whileHover="{ scale: 1.05 }"
            :whileTap="{ scale: 0.95 }">
            <Share2 class="w-5 h-5" />
            <div v-if="showShareToast"
              class="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg whitespace-nowrap flex items-center gap-1.5">
              <Check class="w-3 h-3" /> {{ $t('detail.linkCopied') }}
            </div>
          </motion.button>

          <motion.button @click="store.toggleSaveProgram(program.id, $event)"
            ...
            :aria-label="isSaved ? $t('card.removeBookmark') : $t('card.bookmark')" :aria-pressed="isSaved"
            :whileHover="{ scale: 1.05 }" :whileTap="{ scale: 0.95 }">
            <Star :class="isSaved ? 'text-yellow-500 fill-current scale-110' : 'text-slate-400 dark:text-slate-500'"
      ...
      <div class="flex items-center flex-wrap gap-1.5 mt-6">
        <span class="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide mr-1">{{
          $t('filters.audience') }}:</span>
        <span v-for="aud in program.target_audience" :key="aud"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        <div
          class="bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800/80 p-5 rounded-2xl flex items-center gap-4">
          <Clock class="w-8 h-8 text-slate-400 dark:text-slate-500" />
          <div>
            <p class="text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">{{
              $t('detail.processingTime') }}</p>
            <p class="text-base font-bold text-slate-900 dark:text-slate-100 mt-0.5">{{ program.processing_time }}</p>
          </div>
        </div>

        <a :href="program.official_url" target="_blank"
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              <p class="text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">{{
                $t('detail.officialPortal') }}</p>
              <p class="text-base font-bold text-blue-900 dark:text-blue-400 mt-0.5">{{ $t('detail.visitWebsite') }}</p>
            </div>
          </div>
          <ExternalLink
            class="w-5 h-5 text-blue-900 dark:text-blue-400 group-hover/link:-translate-y-0.5 group-hover/link:translate-x-0.5 transition-transform" />
        </a>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div class="space-y-6">
          <h4
            class="text-sm font-black text-blue-900 dark:text-blue-400 uppercase tracking-widest flex items-center gap-2">
          ...
          <div class="relative pl-6 border-l-2 border-slate-100 dark:border-slate-800 space-y-8 ml-2 mt-2">
            <div v-for="(step, idx) in $tm(`programs.${program.id}.steps`)" :key="idx" class="relative">
              <span
                class="absolute -left-[33px] top-1 w-5 h-5 rounded-full bg-blue-900 dark:bg-blue-800 border-4 border-white dark:border-slate-900 flex items-center justify-center"></span>
              <div class="space-y-1.5">
                <p class="text-[10px] font-black text-blue-900 dark:text-blue-400 uppercase tracking-widest">Step {{
                  Number(idx) + 1 }}</p>
        ...
        <div
          class="space-y-5 bg-slate-50 dark:bg-slate-950 p-6 sm:p-8 rounded-3xl border border-slate-200/50 dark:border-slate-800/50">
          <div class="flex items-center justify-between border-b border-slate-200/60 dark:border-slate-800 pb-4">
            ...
            <span
              class="text-[10px] font-bold text-blue-900 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/30 px-2 py-1 rounded-md">
              {{ checkedList.length }} / {{ program.requirements.length }} {{ $t('detail.prepared') }}
            </span>
          </div>

          <div class="space-y-4 pt-2">
            <label v-for="(req, idx) in $tm(`programs.${program.id}.requirements`)" :key="idx"
              class="flex items-start gap-4 cursor-pointer group/item select-none">
              <input :id="`req-${idx}`" type="checkbox" :checked="checkedList.includes(req)"
      ...
      <div class="flex items-center justify-center">
        <motion.button @click="router.push({ path: '/contact', query: { program: program.id } })"
          class="inline-flex items-center gap-2.5 px-5 py-3 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:text-blue-900 dark:hover:text-blue-400 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50/50 dark:hover:bg-blue-950/20 transition-all cursor-pointer group/suggest print:hidden"
          :whileHover="{ scale: 1.02 }" :whileTap="{ scale: 0.97 }">
          <Lightbulb
            class="w-4 h-4 group-hover/suggest:text-blue-900 dark:group-hover/suggest:text-blue-400 transition-colors" />
          <span class="text-sm font-bold">{{ $t('detail.contactUs') }}</span>
        </motion.button>
      </div>
