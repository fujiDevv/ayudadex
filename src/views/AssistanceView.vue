...
const validateForm = () => {
  ...
  if (!form.value.applicantAddress.trim())
  ...
  if (selectedDoc.value === "authorization") {
    ...
    if (!form.value.repAddress.trim())
      errors.value.repAddress = "Representative Address is required";
    ...
    if (!form.value.explanationAuth.trim())
      errors.value.explanationAuth = "Reason for authorization is required";
    ...
    if (!form.value.explanationRequest.trim())
      errors.value.explanationRequest = "Explanation is required";
  }
...
const letterPreviewHTML = computed(() => {
  ...
  const dateStr = form.value.date
    ? new Date(form.value.date).toLocaleDateString(locale.value, {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
  ...
  if (selectedDoc.value === "authorization") {
    ...
    const requestSubject = patientName
      ? isEn
        ? `REQUEST FOR ASSISTANCE FOR MY ${relToPatient.toUpperCase()}, ${patientName.toUpperCase()}`
        : `HILING PARA SA TULONG SA AKING ${relToPatient.toUpperCase()} NA SI ${patientName.toUpperCase()}`
      : isEn
        ? `REQUEST FOR FINANCIAL/MEDICAL ASSISTANCE`
        : `HILING PARA SA TULONG PINANSYAL / MEDIKAL`;
...
const downloadPDF = () => {
  ...
  if (selectedDoc.value === "authorization") {
    ...
    const govId = form.value.governmentId.trim()
      ? form.value.governmentId.trim()
    ...
    doc.setFontSize(13);
    doc.text(
      isEn ? "AFFIDAVIT OF LOW INCOME" : "SINUMPAANG SALAYSAY NG PAGIGING KAPOS SA KITA",
      105,
      y,
      { align: "center" },
    );
    ...
    doc.setFont("times", "normal");
    doc.setFontSize(10);
    doc.text(isEn ? "Affiant / Declarant" : "Nagsasalaysay (Affiant)", 210 - marginX - 60, y + 5, {
      align: "center",
    });
    y += 20;
    ...
    doc.line(
      marginX,
      y + 1.5,
      marginX + doc.getTextWidth(isEn ? `SUBJECT: ${requestSubject}` : `PAKSA: ${requestSubject}`),
      y + 1.5,
    );
    y += 12;
...
const guidesData = computed(() => {
  ...
  return {
    indigency: {
      ...
      where: isEn
        ? "Your local Barangay Hall (Must have at least 6 months of residency)"
        : "Sa Barangay Hall ng iyong nasasakupan (Dapat residente nang kahit 6 na buwan)",
      timeline: isEn
        ? "Within the day (usually 10 to 30 minutes)"
        : "Sa loob ng araw na iyon (karaniwang 10 hanggang 30 minuto)",
      fees: isEn
      ...
      requirements: [
        {
          id: "ind_req1",
          text: isEn
            ? "At least 1 Valid Government ID proving residence"
            : "Kahit 1 Valid Government ID na nagpapatunay ng iyong address",
        },
        {
          id: "ind_req2",
          text: isEn
            ? "Proof of Residency (e.g. Utility bill under applicant name, land contract, or certification from landlord)"
            : "Patunay ng Pagkatira (hal. Bill ng kuryente/tubig sa pangalan mo, o sertipikasyon mula sa may-ari ng bahay)",
        },
        {
          id: "ind_req3",
          text: isEn
            ? "Community Tax Certificate (Cedula) - usually purchased at the Barangay Hall for ₱5 to ₱30"
            : "Cedula (Community Tax Certificate) - karaniwang binibili sa Barangay Hall sa halagang ₱5 hanggang ₱30",
        },
        {
          id: "ind_req4",
          text: isEn
            ? "First Time Jobseeker Oath (Optional, if applying for jobs to waive all fees)"
            : "Panunumpa bilang First Time Jobseeker (Opsyonal, kung gagamitin sa paghahanap ng trabaho upang maging libre)",
        },
      ],
      steps: [
        {
          id: "ind_step1",
          title: isEn ? "Visit the Barangay Hall" : "Pumunta sa Barangay Hall",
          desc: isEn
            ? "Go to the Barangay Hall of your residence. Bring your own black ballpoint pen and photocopies of your valid ID."
            : "Magtungo sa Barangay Hall ng inyong komunidad. Magdala ng sariling itim na ballpen at kopya ng iyong valid ID.",
        },
        {
          id: "ind_step2",
          title: isEn ? "Secure a Cedula" : "Kumuha ng Cedula",
          desc: isEn
            ? "If you do not have a Cedula for the current calendar year, request one at the cashier/assessment desk. Pay the minor fee based on your income (or declare no income for minimal fee)."
            : "Kung wala ka pang Cedula para sa kasalukuyang taon, kumuha muna sa cashier. Magbayad ng kaunting halaga base sa iyong kita (sabihing walang trabaho kung walang kita).",
        },
        {
          id: "ind_step3",
          title: isEn ? "Fill out the Request Form" : "Punan ang Request Form",
          desc: isEn
            ? 'Ask the desk officer for a Request Form for Certificate of Indigency. Clearly write the purpose of your application (e.g., "DSWD AICS Aid", "BIR Tax Exemption", "Hospitalization Malasakit").'
            : 'Humingi ng Request Form para sa Certificate of Indigency. Isulat nang malinaw ang layunin (hal., "Tulong mula sa DSWD", "BIR Tax Exemption", "Malasakit Center sa Ospital").',
        },
        {
          id: "ind_step4",
          title: isEn ? "Assessment and Verification" : "Pagsusuri at Pagpapatunay",
          desc: isEn
            ? "The barangay secretary will check if you are registered in their local resident listing. They may ask brief questions about your housing status or job situation."
        ...
        {
          id: "ind_step5",
          title: isEn ? "Document Release" : "Paglabas ng Dokumento",
          desc: isEn
            ? "Wait for the Barangay Chairman or authorized officer to sign the certificate. Once stamped with the official seal, it will be released to you. Double-check spelling before leaving."
            : "Maghintay na mapirmahan ng Barangay Chairman ang iyong dokumento. Kapag naselyohan na, ibibigay na ito sa iyo. Suriin ang spelling bago umalis.",
        },
      ],
      tips: [
        isEn
          ? 'Always request the document for "Welfare Assistance" or "Medical Aid" so they can waive the fees entirely.'
          : 'Palaging sabihing gagamitin sa "Tulong Medikal" o "Crisis Assistance" upang alisin ang anumang bayarin sa barangay.',
        isEn
          ? "If you are applying for jobs, present a First-Time Jobseeker certification to invoke Republic Act 11261 for 100% free release."
          : "Kung ikaw ay mag-aapply ng trabaho sa unang pagkakataon, gamitin ang Barangay Certification para sa First-Time Jobseeker sa ilalim ng RA 11261 para maging libre.",
        isEn
          ? "Bring extra photocopies of your IDs. Many government counters will require them, and internet cafes charge a premium for xerox copies."
          : "Magdala ng ekstrang photocopy ng iyong mga ID upang hindi na magbayad ng mahal sa mga xerox machine sa labas.",
      ],
    },
    no_income: {
      ...
      timeline: isEn
        ? "Within the day (usually 1 to 2 hours, depending on RDO queue lines)"
        : "Sa loob ng araw na iyon (karaniwang 1 hanggang 2 oras depende sa haba ng pila)",
      fees: isEn
        ? "FREE or minor Documentary Stamp Tax (approx. ₱30)."
        : "LIBRE o kaunting Documentary Stamp Tax (humigit-kumulang ₱30).",
      requirements: [
        {
          id: "inc_req1",
          text: isEn
            ? "Barangay Certificate of Indigency (Prerequisite! Must specify BIR as the purpose)"
            : "Barangay Certificate of Indigency (Unang kailangan! Dapat BIR ang nakalagay na layunin)",
        },
        {
          id: "inc_req2",
          text: isEn
            ? "Notarized Affidavit of Low Income or No Income (You can generate this in our Document Generator tab!)"
            : "Notarized Affidavit of Low Income o No Income (Maaari mong gawin ito sa tab ng Paggawa ng Dokumento!)",
        },
        {
          id: "inc_req3",
          text: isEn
            ? "Photocopy of 2 Valid Government-Issued IDs (e.g. UMID, Voters ID, PhilHealth, Barangay ID)"
            : "Photocopy ng 2 Valid Government-Issued ID (hal. UMID, Voters ID, PhilHealth, Barangay ID)",
        },
        {
          id: "inc_req4",
          text: isEn
            ? "Completed BIR Form 1917 (Application for Tax Exemption Certificate)"
            : "Pinalagdaang BIR Form 1917 (Application for Tax Exemption Certificate)",
        },
      ],
      steps: [
        {
          id: "inc_step1",
          title: isEn
            ? "Acquire Barangay Certificate of Indigency"
            : "Kumuha muna ng Barangay Certificate of Indigency",
          desc: isEn
            ? 'Get your Barangay Indigency Certificate first. Ensure the purpose listed is "For BIR Certificate of No Income" or "For Tax Exemption Certificate".'
            : "Kumuha muna ng Barangay Certificate of Indigency. Tiyaking ang nakalagay na purpose ay para sa BIR.",
        },
        {
          id: "inc_step2",
          title: isEn ? "Prepare Affidavit of Low Income" : "Ihanda ang Affidavit of Low Income",
          desc: isEn
            ? "Fill out and print the Affidavit of Low Income using our generator. Have it notarized by a notary public (often free/discounted for indigents at Public Attorney's Office - PAO) or signed by your Barangay Captain."
            : "Ihanda at i-print ang Affidavit of Low Income. Ipa-notaryo ito sa pinakamalapit na notaryo o magtanong sa PAO para sa libreng serbisyo.",
        },
        {
          id: "inc_step3",
          title: isEn
            ? "Locate your jurisdiction BIR RDO"
            : "Hanapin ang iyong nakatalagang BIR RDO",
          desc: isEn
            ? "Find out which BIR Revenue District Office (RDO) covers your hometown. You must apply at this specific branch."
            : "Alamin kung aling tanggapan ng BIR RDO ang sumasaklaw sa inyong lugar. Dapat sa sangay na iyon ka mag-apply.",
        },
        {
          id: "inc_step4",
          title: isEn ? "Submit and File at BIR RDO" : "Isumite ang mga papeles sa BIR RDO",
          desc: isEn
            ? "Go to the taxpayer assistance unit at the RDO. Ask for BIR Form 1917, fill it out, and submit all requirements (Indigency, Affidavit, ID copies)."
            : "Magtungo sa taxpayer assistance unit sa RDO. Humingi ng BIR Form 1917, punan ito, at ipasa ang mga dokumento (Indigency, Affidavit, ID copies).",
        },
        {
          id: "inc_step5",
          title: isEn
            ? "Receive Certificate of Tax Exemption"
            : "Tanggapin ang Tax Exemption Certificate",
          desc: isEn
            ? "Wait for the officer to verify that you have no record of tax filing. Once cleared, the Certificate of Tax Exemption (No Income) will be signed and handed to you."
            : "Maghintay na ma-verify ng opisyal na wala kang rekord ng pagbabayad ng buwis. Kapag okay na, ibibigay na ang Certificate of Tax Exemption (No Income).",
        },
      ],
      tips: [
        isEn
          ? "If the BIR RDO is far, check if your local municipal social welfare office (MSWD) will accept just the Barangay Certificate of Indigency as sufficient proof of low income. Many programs are lenient during emergencies."
          : "Kung malayo ang tanggapan ng BIR RDO, itanong sa DSWD o MSWD kung tatanggapin na muna ang Barangay Certificate of Indigency bilang sapat na patunay. Maraming programa ang pumapayag dito sa panahon ng krisis.",
        isEn
          ? "To avoid notary fees, look for the Public Attorney's Office (PAO) near the municipal hall, which offers free notary services for qualified indigent clients."
          : "Upang makaiwas sa bayad sa notaryo, maghanap ng tanggapan ng Public Attorney's Office (PAO) na malapit sa munisipyo, na nagbibigay ng libreng notaryo para sa mga indigent.",
        isEn
          ? "Ensure you bring your own black ink pen as writing instruments are rarely shared at government security counters."
          : "Magdala ng sariling itim na bolpen dahil madalas ay walang mahihiramang panulat sa mga counter ng gobyerno.",
      ],
    },
  };
});
...
<template>
  <div>
    ...
    <div
      class="hidden print:block print:p-12 text-black bg-white font-serif text-sm leading-relaxed min-h-screen"
    >
          ...
          >
            ...
            <form @submit.prevent class="space-y-4">
              ...
              <div>
                ...
                />
                <p v-if="errors.applicantName" class="text-rose-500 text-xs mt-1 font-semibold">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 123 Rizal St., Barangay 4, Manila"
                  class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
              ...
              <div
                v-if="selectedDoc === 'authorization'"
              ...
              >
                <div>
                  ...
                  />
                  <p v-if="errors.repName" class="text-rose-500 text-xs mt-1 font-semibold">
                    {{ errors.repName }}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    v-model="form.repAddress"
                    placeholder="e.g. 123 Rizal St., Barangay 4, Manila"
                    class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    :class="{ 'border-rose-500 dark:border-rose-500': errors.repAddress }"
                  />
                  <p v-if="errors.repAddress" class="text-rose-500 text-xs mt-1 font-semibold">
                    {{ errors.repAddress }}
                  </p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      v-model="form.purposeAuth"
                      placeholder="e.g. claim my AICS cash aid"
                      class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      :class="{ 'border-rose-500 dark:border-rose-500': errors.purposeAuth }"
                    />
                    <p v-if="errors.purposeAuth" class="text-rose-500 text-xs mt-1 font-semibold">
                      {{ errors.purposeAuth }}
                    </p>
                  </div>
                </div>
              ...
              >
                <div class="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    />
                    <p v-if="errors.livelihood" class="text-rose-500 text-xs mt-1 font-semibold">
                      {{ errors.livelihood }}
                    </p>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      v-model="form.governmentId"
                      placeholder="e.g. UMID 12-34567-8"
                      class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    v-model="form.purposeAffidavit"
                    placeholder="e.g. DSWD Cash Assistance / Scholarship"
                    class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    :class="{ 'border-rose-500 dark:border-rose-500': errors.purposeAffidavit }"
                  />
                  <p
                    v-if="errors.purposeAffidavit"
                    class="text-rose-500 text-xs mt-1 font-semibold"
                  >
                    {{ errors.purposeAffidavit }}
                  </p>
                </div>
              </div>
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Hon. Mayor Juan dela Cruz"
                    class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    :class="{ 'border-rose-500 dark:border-rose-500': errors.recipient }"
                  />
                  <p v-if="errors.recipient" class="text-rose-500 text-xs mt-1 font-semibold">
                    {{ errors.recipient }}
                  </p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    />
                    <p v-if="errors.office" class="text-rose-500 text-xs mt-1 font-semibold">
                      {{ errors.office }}
                    </p>
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. City Hall, Manila"
                      class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div class="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      v-model="form.relationshipToPatient"
                ...
                <div>
                  ...
                  <textarea
                    v-model="form.explanationRequest"
                    rows="3"
                    placeholder="e.g. my spouse was laid off last month and our hospital bills have reached ₱50,000"
                    class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    :class="{ 'border-rose-500 dark:border-rose-500': errors.explanationRequest }"
                  ></textarea>
                  <p
                    v-if="errors.explanationRequest"
                    class="text-rose-500 text-xs mt-1 font-semibold"
                  >
                    {{ errors.explanationRequest }}
                  </p>
                </div>
              </div>
        ...
        <div class="lg:col-span-7 space-y-4">
          ...
          >
            ...
            >
              ...
              <div
                v-html="letterPreviewHTML"
                class="preview-document text-justify whitespace-pre-line"
              ></div>
            </motion.div>
          </div>
        </div>
      ...
      <div v-if="activeTab === 'guides'" class="max-w-4xl mx-auto space-y-6">
        ...
        >
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            >
              ...
              <div>
                ...
                <p
                  class="text-xs font-bold text-slate-800 dark:text-slate-200 mt-0.5 leading-normal"
                >
          ...
          <div class="mt-6 border-t border-slate-100 dark:border-slate-800 pt-5">
            <div class="flex items-center justify-between mb-2">
              <span
                class="text-xs font-bold text-slate-650 dark:text-slate-350 flex items-center gap-1.5"
              >
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
          ...
          >
            ...
            <div class="space-y-3">
              <div
                v-for="req in guidesData[selectedGuide].requirements"
                :key="req.id"
                @click="toggleCheckItem(req.id)"
                ...
                :class="
                  checkedItems[req.id]
                    ? 'border-emerald-250 dark:border-emerald-900/60 bg-emerald-50/10 dark:bg-emerald-950/5'
              ...
              >
                <div class="mt-0.5">
                  <CheckCircle
                    ...
                    :class="
                      checkedItems[req.id]
                        ? 'text-emerald-500 fill-emerald-100 dark:fill-emerald-950/50'
                        : 'text-slate-300'
                    "
                  />
                </div>
                <span
                  class="text-xs leading-relaxed font-semibold transition-all"
                  :class="
                    checkedItems[req.id]
          ...
          >
            <div>
              ...
              <div class="space-y-4">
                <div
                  v-for="(step, idx) in guidesData[selectedGuide].steps"
                  :key="step.id"
                ...
                >
                  ...
                  <div class="space-y-1">
                    ...
                    <p
                      class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-normal"
                    >
                      {{ step.desc }}
                    </p>
                  </div>
            ...
            <div class="border-t border-slate-100 dark:border-slate-800 pt-5 space-y-3">
              ...
              >
                <div
                  v-for="(tip, idx) in guidesData[selectedGuide].tips"
                  :key="idx"
