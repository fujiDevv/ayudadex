...
import {
  ClipboardList, BarChart3, Landmark, ArrowRight, Sparkles,
  AlertCircle, Eye, UserCheck, Tag
...
const totalRequirements = computed(() => programsData.reduce((sum, p) => sum + p.requirements.length, 0))
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 animate-fade-in space-y-10 pb-20">
    ...
    <motion.div
      ...
      <div class="relative z-10 max-w-3xl space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 pt-6 max-w-4xl border-t border-slate-200 dark:border-slate-800/60 text-left">
          <div class="p-3 bg-white/45 dark:bg-slate-950/20 rounded-xl border border-slate-200/50 dark:border-slate-800/20 backdrop-blur-xs">
            <span class="block text-2xl font-extrabold text-blue-600 dark:text-blue-400">{{ programsData.length }}</span>
            <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
              {{ $t('analytics.kpiTotalPrograms') }}
            </span>
          </div>
          <div class="p-3 bg-white/45 dark:bg-slate-950/20 rounded-xl border border-slate-200/50 dark:border-slate-800/20 backdrop-blur-xs">
            <span class="block text-2xl font-extrabold text-teal-600 dark:text-teal-400">{{ agenciesList.length }}</span>
            <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
              {{ $t('analytics.kpiAgencies') }}
            </span>
          </div>
          <div class="p-3 bg-white/45 dark:bg-slate-950/20 rounded-xl border border-slate-200/50 dark:border-slate-800/20 backdrop-blur-xs">
            <span class="block text-2xl font-extrabold text-amber-600 dark:text-amber-400">{{ categoryData.length }}</span>
            <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
              {{ $t('analytics.kpiCategories') }}
            </span>
          </div>
          <div class="p-3 bg-white/45 dark:bg-slate-950/20 rounded-xl border border-slate-200/50 dark:border-slate-800/20 backdrop-blur-xs">
            <span class="block text-2xl font-extrabold text-purple-600 dark:text-purple-400">{{ avgRequirements }}</span>
            <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
              {{ $t('analytics.kpiAvgRequirements') }}
            </span>
          </div>
          <div class="p-3 bg-white/45 dark:bg-slate-950/20 rounded-xl border border-slate-200/50 dark:border-slate-800/20 backdrop-blur-xs col-span-2 md:col-span-1">
            <span class="block text-2xl font-extrabold text-rose-600 dark:text-rose-400">{{ avgSteps }}</span>
            <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
    ...
    <section class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xs">
      <div class="flex items-center gap-2 border-b border-slate-100 dark:border-slate-850 pb-4 mb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <!-- Takeaway 1 -->
        <div class="p-5 rounded-2xl border border-slate-100 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-950/30 flex flex-col gap-3">
        ...
        <div class="p-5 rounded-2xl border border-slate-100 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-950/30 flex flex-col gap-3">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-xl bg-blue-550/10 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold">
              2
            </div>
            <h4 class="font-bold text-sm text-slate-900 dark:text-white">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <section
          ...
          <div class="space-y-4.5">
            ...
            >
              <div class="flex justify-between items-center text-xs font-bold mb-1.5 z-10 relative">
                <span class="text-slate-800 dark:text-slate-200 hover:text-blue-650 transition-colors">
                  {{ item.name }}
                </span>
                <div class="flex items-center gap-2">
                  <span class="text-slate-500 dark:text-slate-400">
                    {{ item.count }} {{ $t('analytics.chartTooltipPrograms') }} ({{ Math.round((item.count / programsData.length) * 100) }}%)
                  </span>
                  <button
              ...
              <div class="w-full bg-slate-100 dark:bg-slate-800 h-3.5 rounded-full overflow-hidden relative border border-slate-200/20 dark:border-slate-800/10 shadow-inner">
                <div
              ...
              >
                ...
                >
                  <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider pb-1.5 border-b border-slate-100 dark:border-slate-800">
                    {{ item.name }} {{ $t('analytics.chartTooltipPrograms') }} ({{ item.count }})
                  </div>
                  <div class="max-h-48 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/50">
                    <div
                    ...
                    >
                      <span class="line-clamp-1 flex-grow font-semibold text-slate-800 dark:text-slate-200 group-hover:translate-x-1 transition-transform">
                        {{ $t(`programs.${p.id}.name`) }}
                      </span>
                      <span class="flex items-center gap-1 shrink-0 ml-2 text-[10px] text-blue-500 font-bold opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div class="mt-8 pt-4 border-t border-slate-100 dark:border-slate-850 text-xs text-slate-500 dark:text-slate-400 flex items-center gap-2">
        ...
        <div>
          ...
          <div class="flex flex-col sm:flex-row items-center justify-center gap-8">
            <div class="relative w-44 h-44 shrink-0">
              ...
              <!-- Center details card -->
              <div class="absolute inset-0 flex flex-col items-center justify-center text-center p-3 pointer-events-none select-none">
                <span class="text-2xl font-black text-slate-850 dark:text-white">
                  {{ programsData.length }}
                </span>
                <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                  Programs
                </span>
              </div>
            </div>
          ...
          <div class="mt-6 relative min-h-[90px]">
            <transition name="fade" mode="out-in">
              <div v-if="hoveredCategory" class="p-4 bg-slate-50 dark:bg-slate-950/40 rounded-2xl border border-slate-105 dark:border-slate-800/80 text-left">
                <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2 border-b border-slate-200/50 dark:border-slate-800/50 pb-1.5">
              ...
              <div v-else class="h-full flex items-center justify-center py-6 text-center text-slate-400 dark:text-slate-500 text-xs italic">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <!-- Target Audiences -->
      <section class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xs">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-850 pb-4 mb-6">
          <h3 class="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-2">
        ...
        <div class="space-y-4">
          ...
          >
            <div class="flex items-center justify-between text-xs font-bold mb-1.5">
              ...
              <div class="flex items-center gap-2">
                <span class="text-slate-500 dark:text-slate-450 font-bold bg-slate-100 dark:bg-slate-950 px-2 py-0.5 rounded-md text-[10px]">
                  {{ item.count }} Programs
                </span>
                <button
            ...
            >
              ...
              >
                ...
                <div class="max-h-40 overflow-y-auto divide-y divide-slate-150/40 dark:divide-slate-800/50">
                  <div
                  ...
                  >
                    <span class="truncate flex-grow font-semibold text-slate-800 dark:text-slate-200 group-hover:translate-x-1 transition-transform">
                      {{ $t(`programs.${p.id}.name`) }}
                    </span>
                    <span class="flex items-center gap-0.5 shrink-0 ml-2 text-[10px] text-emerald-500 font-bold opacity-0 group-hover:opacity-100 transition-opacity">
                      {{ $t('detail.viewDetails') }}
                      <ArrowRight class="w-3 h-3" />
                    </span>
                  </div>
      ...
      <!-- Program Tags -->
      <section class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xs flex flex-col justify-between">
        <div>
          <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-850 pb-4 mb-6">
            <h3 class="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-2">
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div class="max-h-40 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/50">
                    <div
                    ...
                    >
                      <span class="truncate flex-grow font-semibold text-slate-850 dark:text-slate-200 group-hover:translate-x-1 transition-transform">
                        {{ $t(`programs.${p.id}.name`) }}
                      </span>
                      <span class="flex items-center gap-0.5 shrink-0 ml-2 text-[10px] text-blue-500 font-bold opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-850 text-xs text-slate-500 dark:text-slate-450 text-left">
