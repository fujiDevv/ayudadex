...
const recommendedPrograms = computed<Program[]>(() => {
  ...
  const scored = (programsData as Program[]).map((program) => {
    ...
    const matchStatus =
      program.target_audience.some(
        (aud) => aud.toLowerCase().includes(statusLower) || statusLower.includes(aud.toLowerCase()),
    ...
    if (
      status.value === "Government Employees" &&
      program.target_audience.includes("Government Employees")
    ) {
      score += 4;
    }
    ...
    demographics.value.forEach((dem) => {
      ...
      const matchDem =
        program.target_audience.some(
          (aud) => aud.toLowerCase().includes(demLower) || demLower.includes(aud.toLowerCase()),
...
<template>
  ...
  >
    ...
    <div
      class="flex items-center justify-between mb-8 border-b border-slate-100 dark:border-slate-800 pb-4"
    >
      <div>
        <h3 class="text-xs uppercase font-medium tracking-wide text-slate-900 dark:text-slate-200">
          {{ $t("finder.title") }}
        </h3>
        <p class="text-xl font-bold text-slate-900 dark:text-slate-200 mt-0.5">
          {{ $t("finder.subtitle") }}
        </p>
      </div>
    ...
    <div class="min-h-[280px] flex flex-col justify-between">
      ...
      <div v-if="step === 1" class="space-y-4 animate-fade-in">
        <h4 id="q1-title" class="text-base font-bold text-slate-900 dark:text-slate-100 mb-4">
          {{ $t("finder.q1") }}
        </h4>
      ...
      <div v-if="step === 2" class="space-y-4 animate-fade-in">
        <h4 id="q2-title" class="text-base font-bold text-slate-900 dark:text-slate-100 mb-4">
          {{ $t("finder.q2") }}
        </h4>
      ...
      <div v-if="step === 3" class="space-y-4 animate-fade-in">
        <h4 id="q3-title" class="text-base font-bold text-slate-900 dark:text-slate-100 mb-4">
          {{ $t("finder.q3") }}
        </h4>
        <div
        ...
        >
          <motion.button
            ...
            class="flex items-center gap-4 p-4 text-left rounded-xl border transition-all duration-205 group active:scale-[0.98] cursor-pointer"
            :class="
              demographics.includes(opt.value)
                ? 'bg-blue-50 dark:bg-blue-950/20 border-blue-900 dark:border-blue-800 text-blue-900 dark:text-blue-400 shadow-sm'
                : 'bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900 hover:border-slate-350 dark:hover:border-slate-700'
            "
            :whileHover="{ scale: 1.02, y: -2 }"
        ...
        <div class="mt-6 border-t border-slate-200 dark:border-slate-800 pt-6">
          <h5
            class="text-sm font-bold text-blue-900 dark:text-blue-400 mb-4 flex items-center gap-2"
          >
          ...
          <div
            v-if="recommendedPrograms.length === 0"
          ...
          >
            ...
            <p class="text-slate-600 dark:text-slate-300 font-medium mt-2">
              {{ $t("finder.noMatchTitle") }}
            </p>
            <p class="text-slate-600 dark:text-slate-300 text-xs mt-1">
              {{ $t("finder.noMatchDesc") }}
            </p>
          </div>
          ...
          <div v-else class="space-y-3 max-h-[250px] overflow-y-auto pr-1">
            ...
            >
              <div class="flex-1 pr-4">
                ...
                <h6 class="text-sm font-bold text-slate-900 dark:text-slate-100 mt-1 line-clamp-1">
                  {{ prog.name }}
                </h6>
              </div>
              <motion.button
      ...
      <!-- Navigation buttons -->
      <div
        class="flex items-center justify-between mt-8 border-t border-slate-200 dark:border-slate-800 pt-4"
      >
        <motion.button
          v-if="step > 1"
          @click="prevStep"
