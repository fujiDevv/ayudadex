...
const getCategoryIcon = (category: string) => {
  ...
  if (
    ...
    return Coins;
  if (norm.includes("education") || norm.includes("scholarship") || norm.includes("student"))
    return GraduationCap;
  if (norm.includes("housing") || norm.includes("property")) return Home;
  if (norm.includes("employment") || norm.includes("livelihood") || norm.includes("work"))
    return Briefcase;
  return Users;
};
</script>
...
<template>
  ...
  >
    ...
    <div
      v-if="viewMode === 'list'"
      class="flex flex-col md:flex-row md:items-center justify-between w-full gap-4 sm:gap-6"
    >
      <div class="space-y-3 flex-grow md:max-w-[65%]">
        <div class="flex items-center gap-2 flex-wrap">
          <span
            :class="getAgencyStyles(program.agency)"
            class="px-2.5 py-0.5 text-[10px] font-bold uppercase rounded-md tracking-wider border"
          >
            {{ program.agency }}
          </span>
          <span
            class="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-[10px] font-bold uppercase rounded-md tracking-wider flex items-center gap-1"
          >
            <component
              :is="getCategoryIcon(program.category)"
        ...
        <p
          class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 line-clamp-2 leading-relaxed"
        >
        ...
        <div class="flex items-center flex-wrap gap-1.5 pt-1">
          <span
            class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide mr-1"
            >For:</span
          >
          <span
      ...
      <div
        class="flex flex-col items-stretch gap-3 w-full md:w-[220px] shrink-0 pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-slate-100 dark:border-slate-800/80 md:pl-6"
      >
        <!-- Bookmark & Readiness Count -->
        <div class="flex items-center justify-between gap-4 w-full">
          <div class="flex flex-col text-left">
            <span
              class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide"
              >{{ $t("card.readiness") }}</span
            >
            <span
              class="text-xs font-extrabold text-blue-600 dark:text-blue-400 flex items-center gap-1 mt-0.5"
            >
          ...
          <motion.button
            ...
            :aria-label="
              (isSaved ? $t('card.removeBookmark') : $t('card.bookmark')) +
              ' ' +
              $t(`programs.${program.id}.name`)
            "
          ...
          >
            <Star
              :class="
                isSaved ? 'text-yellow-500 fill-current' : 'text-slate-400 dark:text-slate-500'
              "
              class="w-4 h-4"
            />
          </motion.button>
        ...
        >
          <div
            ...
            :class="[
              preparedPercent === 100
                ? 'bg-emerald-500 sm:bg-gradient-to-r sm:from-emerald-500 sm:to-teal-500'
                : 'bg-blue-600 sm:bg-gradient-to-r sm:from-blue-600 sm:to-indigo-600',
            ]"
    ...
    <div v-else class="flex flex-col h-full justify-between gap-5 w-full">
      <div class="space-y-4">
        ...
        <!-- Title -->
        <h3
          class="text-lg sm:text-xl font-bold text-slate-900 dark:text-slate-100 leading-tight text-left"
        >
          {{ $t(`programs.${program.id}.name`) }}
        </h3>
        ...
        <!-- Description -->
        <p
          class="text-sm text-slate-600 dark:text-slate-300 leading-relaxed font-normal line-clamp-3 text-left"
        >
          {{ $t(`programs.${program.id}.description`) }}
        </p>
      ...
      <div class="space-y-4 border-t border-slate-100 dark:border-slate-800 pt-4 mt-auto">
        <div>
          <div class="flex items-center justify-between mb-1.5">
            <span
              class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide flex items-center gap-1"
            >
              <FolderOpen class="w-3.5 h-3.5" /> {{ $t("card.readiness") }}
            </span>
            <span
              class="text-xs font-extrabold text-blue-600 dark:text-blue-400 flex items-center gap-1"
            >
              <CheckCircle2
                v-if="preparedPercent === 100"
